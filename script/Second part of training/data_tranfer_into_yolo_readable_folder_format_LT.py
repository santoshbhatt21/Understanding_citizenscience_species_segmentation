...
SOURCE_IMAGES = r"E:/Santosh_master_thesis/classified_Leaves"  # root folder of images to process
# sibling <class>_labels folders
SOURCE_LABELS = r"E:/Santosh_master_thesis/Generated_YOLO_Seg_Labels_Leaves_inverted"
TARGET_ROOT = r"E:/Santosh_master_thesis/DATA_YOLO11_cleaned_labels/"  # output
SPLIT_RATIO = 0.8
SEED = 42
...
def _resolve_label_root(class_name: str) -> str:
    """Return the label root for a class, trying '<class>_labels' then '<class>'."""
    cand1 = os.path.join(SOURCE_LABELS, f"{class_name}_labels")
    cand2 = os.path.join(SOURCE_LABELS, class_name)
    if os.path.isdir(cand1):
        return cand1
    if os.path.isdir(cand2):
        return cand2
    # warn; default to cand2
    print(f"⚠️ No label folder for class '{class_name}' under '{SOURCE_LABELS}'. Tried: {cand1}, {cand2}")
    return cand2
...
def collect_pairs_for_class(class_name: str) -> List[Tuple[str, str, str]]:
    ...
    class_lbl_root = _resolve_label_root(class_name)
    pairs = []
