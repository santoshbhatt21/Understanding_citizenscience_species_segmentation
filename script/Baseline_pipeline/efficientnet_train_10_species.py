...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Set your paths
DATA_PATH = r"E:/Santosh_master_thesis/Understanding_citizenscience_species_segmentation/Data"
CHECKPOINT_DIR = r"E:/Santosh_master_thesis/Checkpoints_efficientnet_10_species"
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
