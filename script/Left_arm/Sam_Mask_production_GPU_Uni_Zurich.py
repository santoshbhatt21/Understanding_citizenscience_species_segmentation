...
model_path = 'E:/Santosh_master_thesis/Understanding_citizenscience_species_segmentation/script/Left_arm/Checkpoint_leftarm_4k_oneCLR/best_model_148_0.33.pth'
...
def process_folder(subdir_path, folder_idx, threshold_value, num_sampled_points):
    target_class = folder_idx
    parent_dir = os.path.dirname(subdir_path)
    subdir_name = os.path.basename(subdir_path)

    # Mask folder directly under parent, e.g. Data/abies_mask
    save_folder = os.path.join(parent_dir, f"{subdir_name}_mask")
    os.makedirs(save_folder, exist_ok=True)

    image_paths = [
        os.path.join(subdir_path, image_name)
        for image_name in os.listdir(subdir_path)
        if os.path.isfile(os.path.join(subdir_path, image_name))
        and image_name.lower().endswith(patterns)
    ]

    for i in range(0, len(image_paths), Batch_size):
        batch_paths = image_paths[i:i + Batch_size]
        process_images_in_batch(
            batch_paths, target_class, threshold_value, num_sampled_points, save_folder
        )
    ...
    try:
        ...
        for root, dirs, files in os.walk(base_dir):
            for folder_idx, subdir in enumerate(sorted(dirs)):
                ...
                process_folder(subdir_path, folder_idx, Threshold_value, No_of_sampled_points)
    except Exception as e:
