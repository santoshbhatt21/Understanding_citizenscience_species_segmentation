...
PANEL_PATH = OUT_ROOT / "all_species_2x5_triplets_no_inverted_masks_legend2.png"
...
def colorize_mask_from_gray(mask_gray: np.ndarray, species_name: str) -> Image.Image:
    h, w = mask_gray.shape
    m_rgb = np.zeros((h, w, 3), dtype=np.uint8)
    fg_mask = mask_gray > 0

    m_rgb = np.zeros((*fg_mask.shape, 3), dtype=np.uint8)
    m_rgb[~fg_mask] = BG_COLOR   # light blue background
    m_rgb[fg_mask]  = FG_COLOR   # dark blue foreground
    return Image.fromarray(m_rgb, "RGB")
    ...
    for species_dir in sorted(IMAGE_ROOT.iterdir()):
        ...
        for img_path in img_candidates:
            ...
            if (
                ...
                # FG not clearly better than inverted
                (iou_fg - iou_inv) < MIN_IOU_MARGIN
            ):
            ...
            mask_img = colorize_mask_from_gray(m_np, species_name)
        ...
        if not chosen:
            print(
                f"[WARN] No suitable sample found for species '{species_name}'")
